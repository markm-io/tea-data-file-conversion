...
def process_fixed_width_file(input_file, schema_config, skip_header=True):
    """
    ...
    df = pd.read_fwf(input_file, colspecs=colspecs, header=None, names=col_names, skiprows=skiprows)
...
def process_file(input_file, output_file=None, schema_folder=None):
    """
    ...
    base_folder = schema_folder if schema_folder is not None else "."
    ...
    df = process_fixed_width_file(input_file, schema_config, skip_header=True)
...
def csv_to_schema_yaml(csv_file, yaml_output_file=None):
    ...
    for index, row in df.iterrows():
        ...
        output_field_value = str(row[output_field_col])
        field_entry = {
            "start": start_value,
            "end": end_value,
